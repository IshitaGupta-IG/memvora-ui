...
export default function ChatWindow() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "assistant",
      content: "Ask me to find saved links, screenshots, notes, fuzzy matches, recurring ideas, or themes across your memories.",
    },
  ]);
  ...
  if (!open) {
    return (
      <button
        className="fixed inset-x-3 bottom-3 z-40 inline-flex items-center justify-center gap-3 rounded-2xl bg-slate-950 px-5 py-4 text-sm font-semibold text-white shadow-2xl shadow-brand-900/25 transition hover:-translate-y-0.5 hover:bg-brand-700 sm:inset-x-auto sm:bottom-5 sm:right-5"
  ...
  return (
    <section id="ai-chat" className="fixed inset-x-0 bottom-0 top-16 z-40 flex flex-col overflow-hidden rounded-t-3xl border border-white/80 bg-white shadow-2xl shadow-brand-900/20 sm:inset-x-auto sm:bottom-5 sm:right-5 sm:top-auto sm:h-[min(680px,calc(100vh-2.5rem))] sm:w-[min(430px,calc(100vw-2.5rem))] sm:rounded-3xl">
      <div className="border-b border-white/70 bg-white/80 p-4 sm:p-5">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">Search and ask your memories</p>
            </div>
          </div>
      ...
      <div className="flex-1 space-y-4 overflow-y-auto bg-gradient-to-b from-brand-50/60 to-slate-50 p-3 sm:p-4">
        {messages.map((message, index) => (
          <div key={`${message.role}-${index}`} className={`flex gap-3 ${message.role === "user" ? "justify-end" : "justify-start"}`}>
            ...
            <div className={`min-w-0 max-w-[86%] break-words rounded-2xl px-4 py-3 text-sm leading-6 sm:max-w-[82%] ${message.role === "user" ? "bg-slate-950 text-white shadow-lg shadow-brand-700/20" : "bg-white text-slate-700 shadow-sm"}`}>
      ...
      <form onSubmit={handleSubmit} className="flex gap-2 border-t border-white/70 bg-white/90 p-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] sm:gap-3 sm:p-4">
